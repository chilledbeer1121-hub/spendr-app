...
function Dashboard() {
  ...
  const stats = useMemo(() => {
    ...
    return {
      total,
      remaining: Math.max(0, salary - total),
      biggestCat,
      biggestAmt,
  ...
  return (
    <div className="px-4 pt-6 pb-4 md:px-8 md:pt-8 max-w-5xl mx-auto">
      ...
      {/* Summary cards 2x2 mobile, 4x desktop */}
      <div className="grid grid-cols-2 gap-3 md:grid-cols-4 md:gap-4">
        <SummaryCard
          label="Spent this month"
          value={formatCurrency(stats.total, currency)}
          sub={`${pctOfSalary(stats.total, salary).toFixed(1)}% of salary`}
          tone="default"
        />
        <SummaryCard
          label="Remaining"
          value={formatCurrency(stats.remaining, currency)}
          sub={`${savingRate(salary, stats.total).toFixed(0)}% saving rate`}
          tone="success"
        />
        <SummaryCard
          label="Biggest category"
          value={stats.biggestCat?.name ?? "—"}
          sub={stats.biggestAmt ? formatCurrency(stats.biggestAmt, currency) : "Nothing yet"}
          tone="default"
        />
        <SummaryCard
          label="Expenses"
      ...
      <Card className="mt-4 p-4 md:p-6 bg-card border-border">
        ...
        {donutData.length === 0 ? (
          <div className="py-8 text-center text-sm text-muted-foreground">
            ...
            <Link to="/add" className="text-primary font-medium">
              Log your first one →
            </Link>
          </div>
        ) : (
          <SpendDonut data={donutData} centerLabel={formatCurrency(stats.total, currency)} centerSub="spent" />
        )}
      </Card>
...
function SummaryCard({ label, value, sub, tone }: { label: string; value: string; sub: string; tone: "default" | "success" }) {
  return (
    <Card className="p-3.5 md:p-4 bg-card border-border">
      ...
      <div className={`mt-1.5 font-display text-lg md:text-xl font-bold tabular-nums truncate ${tone === "success" ? "text-success" : ""}`}>
        {value}
      </div>
  ...
  return (
    <div className="px-4 pt-10 pb-4 max-w-md mx-auto text-center">
      ...
      <p className="mt-2 text-sm text-muted-foreground">
        First, set your monthly take-home salary. Every percentage on the dashboard is calculated against it.
      </p>
      <Button className="mt-6" onClick={() => nav({ to: "/settings" })}>
        Set my salary
      </Button>
